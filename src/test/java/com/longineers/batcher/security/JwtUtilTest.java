...
@TestPropertySource(properties = {          // fallback in case the file isn't loaded
        "jwt.secret=TestJwtSecretKeyChangeMeThis32CharKey",
        "jwt.expiry-time=600000"           // 10 min
})
class JwtUtilTest {
     ...
     *  -------------------------------------------------------------------- */
    @Test
    @DisplayName("generateToken → validateToken returns true for same user")
    ...
    void tokenIsNotExpiredImmediately() throws Exception {
        ...
        // Actually we can just invoke the public validateToken which internally checks expiry.
        boolean valid = jwtUtil.validateToken(token, userDetails);
        assertThat(valid).isTrue();
    }
     ...
     *  -------------------------------------------------------------------- */
    @Test
    @DisplayName("validateToken throws ExpiredJwtException when token is expired")
     ...
     *  -------------------------------------------------------------------- */
    @Test
    @DisplayName("validateToken fails when usernames differ")
    void tokenFailsForDifferentUser() {
        ...
        boolean valid = jwtUtil.validateToken(token, bob);
    ...
    class InvalidTokenTests {
        ...
        @Test
        @DisplayName("validateToken throws SignatureException for token with wrong signature")
        void validateTokenWithWrongSignature() throws Exception {
            ...
            assertThatThrownBy(() -> jwtUtil.validateToken(wrongSignatureToken, userDetails))
    ...
    class NullInputTests {
        ...
        @Test
        @DisplayName("validateToken throws exception when UserDetails is null")
        void validateTokenWithNullUserDetails() {
            var userDetails = Mockito.mock(org.springframework.security.core.userdetails.UserDetails.class);
            Mockito.when(userDetails.getUsername()).thenReturn(testUsername);
            String token = jwtUtil.generateToken(userDetails);

            assertThatThrownBy(() -> jwtUtil.validateToken(token, null))
                    .isInstanceOf(NullPointerException.class);
        }

        @Test
        @DisplayName("validateToken throws exception when token is null")
    ...
    class EdgeCaseTests {
        ...
        @Test
        @DisplayName("validateToken returns false when UserDetails username differs from token subject")
        void validateTokenUsernameMismatch() {
            ...
            String token = jwtUtil.generateToken(alice);
            boolean valid = jwtUtil.validateToken(token, charlie);
    ...
    class TokenStructureTests {
        ...
        void tokenValidationChecksUsernameAndExpiration() throws Exception {
            ...
            assertThat(jwtUtil.validateToken(validToken, userDetails)).isTrue();
